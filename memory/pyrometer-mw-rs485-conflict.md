...
**Эта защита пропала и возвращена 24.09.2026 — только для МВ.** Экспорт из CODESYS в `279141b` (04.09) откатил проект к версии до 26.08 и стёр и квитирование, и фильтр. Пользователь поймал последствие: ПЛК стартовал при выключенном генераторе, `MW_GEN` ушёл в ошибку и без перезагрузки ПЛК генератор больше не подключался. CODESYS после ошибки слейва перестаёт его опрашивать, пока ошибку не квитируют. Сейчас в `PRG_Microwave.st`: `fbCommOk` (TON 4 с на `NOT MW_GEN.xError`, от него `xMwCommError`), `fbConfirmTick` (импульс `MW_GEN.xConfirmError` раз в 4 с, пока есть ошибка), плюс по фронту восстановления связи `stMw.iLastSetpointRaw := -1`, чтобы уставка, записанная в мёртвый генератор, ушла повторно. Команды ON/Preheat при восстановлении **не** переотправляются: генерация могла бы возобновиться без оператора, и это открытый вопрос из [[ardis-cvdcore-open-work]]. `PYRO_RXT` и `TURBO_PUMP` так и остались без квитирования. Если экспорт из CODESYS снова перезапишет `.st`, первым делом проверить, на месте ли `fbConfirmTick`.
