...
**Квитирование МВ — только по кнопке RECONNECT (24.09.2026).** Автоквитирование раз в 4 с, возвращённое утром того же дня после потери в `279141b`, пользователь заменил ручным: в строке Preheat секции Microwave вместо серой надписи «Not connected» кнопка `MWReconnect`, видна только пока генератор не отвечает (`!GeneratorAnswering`), шлёт импульсом бит `0x0008` в `awHolding[192]` (`PLC210MicrowaveClient.RequestGeneratorReconnect`, как RESET — одна запись, повтор при сбое записи). `PRG_Microwave`: `fbConfirmPulse : TP` 500 мс по фронту бита → `MW_GEN.xConfirmError`. Оставлены `fbCommOk` (связь считается живой после 4 с без ошибки, чтобы HMI не мигал после квитирования) и повторная отправка уставки по фронту восстановления связи (`stMw.iLastSetpointRaw := -1`). ON/Preheat при восстановлении не переотправляются. Кнопка внутри `Microwave_groupBox`, а он выключен вне Manual Run — значит и RECONNECT вне сессии серый. `PYRO_RXT` и `TURBO_PUMP` без квитирования вовсе.
